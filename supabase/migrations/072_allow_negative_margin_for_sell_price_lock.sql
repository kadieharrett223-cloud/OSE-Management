-- Allow negative margins when preserving sell prices after tariff changes.
-- This enables sell_price = final_cost / (1 - margin) for margins below 0.
...
BEGIN
  ...
  IF COALESCE(NEW.margin, 0) < 1 THEN
    NEW.sell_price := NEW.cost_with_shipping / (1 - COALESCE(NEW.margin, 0));
  ELSE
