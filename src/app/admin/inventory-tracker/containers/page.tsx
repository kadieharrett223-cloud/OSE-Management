...
export default function InventoryContainersPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      setProducts(((productsResult.data || []) as any[]).map((p) => ({ id: p.id, name: p.name })));
      setContainers((containersResult.data || []) as ContainerRow[]);
    } catch (error: any) {
      alert(error?.message || "Failed to load containers");
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="mx-auto max-w-6xl space-y-6">
          <header className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h1 className="text-2xl font-semibold">Inventory Containers</h1>
                <p className="mt-1 text-sm text-slate-600">Manage container status and which inventory products are on each container.</p>
              </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={loadAll} className="rounded-md border border-slate-300 px-3 py-1 text-sm hover:bg-slate-50">
                Refresh
              </button>
            </div>
            ...
            {loading ? (
              <p className="mt-4 text-sm text-slate-600">Loading containers...</p>
            ) : containers.length === 0 ? (
