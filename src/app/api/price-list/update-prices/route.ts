...
export async function POST(request: NextRequest) {
  try {
    ...
    // List price updates from spreadsheet
    const updates = [
      // 2 Post Lifts
    ...
    return NextResponse.json({
      success: true,
      message: `Updated ${updatedCount} items. ${notFoundCount} items not found.`,
      notFound,
    });
    
  } catch (error: any) {
    console.error("Error updating list prices:", error);
    return NextResponse.json(
