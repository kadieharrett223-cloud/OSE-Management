      ...
      if (!response.ok) {
        ...
        if (isCheckoutScopeApprovalError(response.status, text)) {
          return NextResponse.json(
            {
              error:
                "Shopify checkout access is not approved for this app. Normal Shopify sign-in will work, but abandoned carts cannot be loaded unless Shopify approves read_checkouts for this app.",
            },
            { status: 403 }
          );
        }
