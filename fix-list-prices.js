// Apply migration and update prices
...
async function applyMigration() {
  ...
  const migration = fs.readFileSync('supabase/migrations/010_preserve_manual_list_price.sql', 'utf8');
...
async function updatePrices() {
  ...
  console.log(`\nUpdating ${updates.length} list prices...`);
  let count = 0;
  ...
  for (const update of updates) {
    ...
    if (!error) {
      count++;
      if (count % 10 === 0) console.log(`Updated ${count}/${updates.length}...`);
    } else {
      console.error(`❌ ${update.item_no}: ${error.message}`);
    }
  }
  
  console.log(`✅ Updated ${count} list prices successfully!`);
}
